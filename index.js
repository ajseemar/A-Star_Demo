...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const MazeSolverDemo = new Game(c.width, c.height, 100);
...
class aStar {
    ...
    update() {
        if (this.openSet.length > 0) {
            ...
            // debugger
            const neighbors = current.neighbors.filter(obj => !Object.keys(current.walls).includes(Object.keys(obj)[0]));
            // debugger
            neighbors.map(obj => Object.values(obj)[0]).forEach(neighbor => {
                if (!this.closedSet.includes(neighbor)) {
                    let tentativeG = current.node.g + 1;
                    if (this.openSet.includes(neighbor)) {
                        if (tentativeG < neighbor.node.g) neighbor.node.g = tentativeG;
                    } else {
